    ...
    default Page<Post> findAllByFilters(PostFilterParam filter, Pageable pageable) {
        return findAll((root, criteriaQuery, criteriaBuilder) -> {
            ...
            if (filter.getPriceStarts() != null && filter.getPriceEnds() != null) {
                Predicate priceStartsPredicate = criteriaBuilder.greaterThanOrEqualTo(rentHouse.get("roomPrice"), filter.getPriceStarts());
                Predicate priceEndsPredicate = criteriaBuilder.lessThanOrEqualTo(rentHouse.get("roomPrice"), filter.getPriceEnds());
            ...
            if (filter.getGender() != null && !filter.getGender().isEmpty()) {
                ...
                for (Long c : filter.getGender()) {
                    genderPredicateList.add(criteriaBuilder.equal(rentHouse.get("gender").get("id"), c));
                }
                Predicate genderIdPredicate = criteriaBuilder.or(genderPredicateList.toArray(new Predicate[0]));
