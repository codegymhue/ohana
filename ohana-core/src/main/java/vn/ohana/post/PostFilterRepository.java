    ...
    default Page<Post> findAllByFilters(PostFilterParam filter, Pageable pageable) {
        return findAll((root, criteriaQuery, criteriaBuilder) -> {
            ...
            if (filter.getPriceStarts() != null && filter.getPriceEnds() != null) {
                Predicate priceStartsPredicate = criteriaBuilder.greaterThan(rentHouse.get("room_price"), filter.getPriceStarts());
                Predicate priceEndsPredicate = criteriaBuilder.lessThan(rentHouse.get("room_price"), filter.getPriceEnds());
